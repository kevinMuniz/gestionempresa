...
public class ingreso_modificacion {
    ...
    public void ingreso_nombreMateriasPrimasAuxiliares(String rucx, int ano,String descripcion ,String ND0 , String unidadMedida ,String cantidad_ND1 ,String valor_ND1,String cantidad_ND2 ,String valor_ND2 ,String cantidad_ND3 ,String valor_ND3 ,String ND4 )
    {
        try{                    
            Class.forName("org.postgresql.Driver");
            Connection conex = DriverManager.getConnection(cadena,usuario,contrasenia);
            java.sql.Statement st = conex.createStatement();
            String sql = "select InsertarMateriasPrimasAuxiliares('"+rucx+"',"+ano+",array["+descripcion+"],array["+ND0+"],array["+unidadMedida+"],array["+cantidad_ND1+"],array["+valor_ND1+"],array["+cantidad_ND2+"],array["+valor_ND2+"],array["+cantidad_ND3+"],array["+valor_ND3+"],array["+ND4+"])";
            ResultSet resultSet = st.executeQuery(sql);                        
            resultSet.close();                
            st.close();
            conex.close();                
